...
use std::sync::Arc;
use svc_scheduler_client_grpc::client::Itinerary;
...
const ITINERARY_KEEPALIVE_DURATION_SECONDS: i64 = 120;
...
pub trait ItineraryPool {
    ...
    async fn store_itinerary(&mut self, draft_itinerary: &DraftItinerary) -> Result<i64, CacheError>
    where
        Self: Send + Sync + 'async_trait,
    {
        ...
        let mut attempt = 0;
        let Some(itinerary_id) = (0..UUID_COLLISION_RETRIES)
            .collect()
            .iter()
            .find_map(async move {
            ...
            match connection.expire(key.clone(), ITINERARY_KEEPALIVE_DURATION_SECONDS).await {
                Ok(Value::Int(1)) => Some(itinerary_id),
                Ok(value) => {
                    cache_error!(
        ...
        Self: Send + Sync + 'async_trait,
    {
        ...
        match result {
            ...
            Ok(result) => {
                cache_error!(
                    "(ItineraryPool get_task_data) unexpected redis response from hget: {:?}",
                    result
                );
