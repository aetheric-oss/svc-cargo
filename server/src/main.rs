...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "List all cargo-accessible vertiports successfully", body = [rest_types::Vertiport]),
...
pub async fn query_vertiports(
    ...
    let response = grpc_clients.storage.vertiports(request).await;
    if response.is_err() {
        //  TODO more granular errors
        return Err(StatusCode::BAD_REQUEST);
    }

    let mut ret: Vec<rest_types::Vertiport> = vec![];
    for v in response.unwrap().into_inner().vertiports {
        ret.push(rest_types::Vertiport {
            id: v.id,
            label: v.label,
            latitude: v.latitude,
...
async fn main() -> Result<(), tonic::transport::Error> {
    ...
    let grpc_clients = grpc_clients::GrpcClients::create().await?;
